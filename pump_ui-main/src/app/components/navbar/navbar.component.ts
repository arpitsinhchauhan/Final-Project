...
export class NavbarComponent implements OnInit {
  ...
  constructor(location: Location, private element: ElementRef, private router: Router,
    private dialog: MatDialog, private userServiceService: UserServiceService, private commonService: CommonService
  ) {
  ...
  logout(): void {
    this.userServiceService.closeServer().subscribe({
      next: () => {
        // After server disconnect, navigate to login
        this.router.navigate(['/']);
      },
      error: () => {
        // this.notificationService.failure('Server close failed:', err);
        // Still navigate to login if needed
        this.router.navigate(['']);
      }
    });
    // this.router.navigate(['/']);
  ...
  minimizeSidebar() {
    ...
    if (isMobile) {
      this.hideSidebar(); // on mobile, hide sidebar instead
      return;
    }
  ...
  sidebarOpen() {
    ...
    } else if (body.classList.contains('off-canvas-sidebar') && wrapperFullPage) {
